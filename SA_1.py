...
def iteration(OrderIDlist,OrderFile,storeFile,disatnceFile,warehouseDataframe):
    for OrderID in OrderIDlist[:100].drop_duplicates():
        try:
            ...
            x,cost = annealingoptimize(x,storeNum,distances,priority,variableID,T=1.0,cool=0.8,iterMax=10)
            x_all.append(x)
...
#    import threading
#    threads = []
#    t1 = threading.Thread(target=stroeTable,args=(goodsTable,warehouse, storeFile,))
#    threads.append(t1)
#    t2 = threading.Thread(target=disGet,args=(disatnceFile, warehouseDataframe, address))
#    threads.append(t2)
...
#    cost,error = iteration(OrderIDlist,OrderFile,storeFile,disatnceFile,warehouseDataframe)
#def main(OrderIDlist,OrderFile,storeFile,disatnceFile,warehouseDataframe):
    for OrderID in OrderIDlist[:100].drop_duplicates():
        try:
            Order = OrderfromTable(OrderID, OrderFile,)
            goodsTable, address = OrderInfo(Order)
